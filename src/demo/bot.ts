...
if (!env.SEVEN_NIGHTS_NO_POLL) {
  ...
  /**
   * 409 от getUpdates значит одно: этот токен уже читает кто-то другой.
   * Раньше процесс валился со стеком grammY, и в логе это выглядело как поломка
   * бота, хотя поломка — в том, что запущены двое. Выходим с отдельным кодом и
   * понятной строкой; в юните этот код помечен как «не перезапускать», иначе
   * systemd устроит бесконечную драку двух экземпляров за одну связь.
   */
  process.on('unhandledRejection', (e: any) => {
    if (e?.error_code === 409) {
      console.error('Этот токен уже читает другой экземпляр бота. Останавливаюсь, чтобы не драться за связь.')
      process.exit(3)
    }
    throw e
  })
  ...
  bot.start().catch((e: any) => {
    ...
    console.error('long polling остановлен:', e instanceof Error ? e.message : e)
    process.exit(1)
  })
